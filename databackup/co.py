...
def regenerate(name):
	...
	for i in g:
		j=i.split(',')
		l+=[vm(-1,float(j[1]),j[0],float(j[2]))]
	return l
...
def createVmList(n):
	...
	for i in range(n):
		...
		if (rand.uniform(0.0,9.0)<=5.0 or not ma):
			...
			del ma[i]
		l+=[v]
	name="L"+str(time.strftime("%H:%M:%S"))+".txt"
	f = open(name,'w')
	s=""
	for i in l:
		s+=i.name+","+str(i.ram)+","+str(i.cpu)+"\n"
	f.write(s)
	f.close()
	return l
...
def writeToFile(w,count,fname):
	jobs=0
	name = fname + str(count) + '.txt'
	f = open(name,'w')
	stri=""
	for x in w.host_list:
...
# lll =regenerate("L14:50:26.txt")
# print map(lambda c: (c.name,c.ram,c.cpu ),lll) 
