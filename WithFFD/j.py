...
col_n = 7
vm_colors=[colors[i%col_n] for i,z in enumerate(vm_list)];
...
def createGraph(w):
	...
	for j,x in enumerate(w.host_list):
		...
		for i,z in enumerate(activeVms):
			...
			g[1][i][j] = ucpu
			vmc[i][j] = vm_colors[z]
		# 	print "COLORS ",i,j,z,vm_colors[z] , vmc[i][j]

		# for i in range(rows):
		# 	print "PRINTING ",vmc[i][j]
...
################################################################################################################################
def ffdCost(r,c):
	return 0.5*r+ 512.0*c

def ffd(nod):
	nod.vm_list=filter(lambda c: c.level!=0,nod.vm_list)
	nod.vm_list=sorted(nod.vm_list,cmp=lambda x,y:(-1 if ffdCost(x.ram/(2**(x.level-1)),x.cpu/(2**(x.level-1)))-ffdCost(y.ram/(2**(y.level-1)),y.cpu/(2**(y.level-1)))<0 else 1),reverse=True)
	for i,x in enumerate(nod.host_list):
		nod.host_list[i].usedC=nod.host_list[i].usedR=0
		nod.host_list[i].vm=[]
	nod.host_list.sort(cmp=lambda x,y:(-1 if ffdCost(x.ram,x.cpu)-ffdCost(y.ram,y.cpu)<0 else 1),reverse=True )
	print map(lambda c:(c.name,c.cpu,c.usedC) ,nod.host_list);
	for i,x in enumerate( nod.vm_list):
		if x.level==0:
			continue
		n_r=x.ram/(2**(x.level-1))
		n_c=x.cpu/(2**(x.level-1))
		fitt=False
		for j,y in enumerate( nod.host_list):
			if y.status!=1:
				continue
			if y.ram-y.usedR>=n_r and y.cpu-y.usedC>=n_c and ((n_r+y.usedR<0.8*y.ram and n_c+y.usedC<0.8*y.cpu) or ((y.usedR/1024.0>=y.usedC and n_r/1024.0<=n_c) or (y.usedR/1024.0<=y.usedC and n_r/1024.0>=n_c) )):
				nod.host_list[j].usedR+=n_r
				nod.host_list[j].usedC+=n_c
				nod.host_list[j].vm+=[i]
				nod.vm_list[i].host=y.name
				fitt=True
				break
		if fitt==False:
			succ=False
			for j, y in enumerate(nod.host_list):
				if y.status!=1:
					nod.host_list[j].status=1
					if y.ram-y.usedR>=n_r and y.cpu-y.usedC>=n_c :
						nod.host_list[j].usedR+=n_r
						nod.host_list[j].usedC+=n_c
						nod.host_list[j].vm+=[i]
						nod.vm_list[i].host=y.name
						succ=True
			if succ==False:	
				print (x.name,x.ram,x.cpu)
				print len(nod.vm_list)-i
				print map(lambda c:(c.name,c.ram/(1<<(c.level-1)),c.cpu/(1<<(c.level-1)) ) ,nod.vm_list[i:])
				print "\n\n\nFAIL_IN_FFD_OF_ALL_AT_ONCE\n\n\n"
			...
			for j,x in enumerate(nod.host_list):
				...
				for k,z in enumerate(vic2_vms):
					...
					if _ram+freeR>=n_r and _cpu+freeC>=n_c:
						print freeR,freeC,'------',_ram,_cpu,"----",n_r,n_c
...
def poten():
	...
	return potential_vics

createGraph(test1)
# _level=0
# while _level<=0:
# 	potential_vics=poten()
# 	test1.victim=potential_vics[0]
# 	try:
# 		help(test1)
# 		createGraph(test1)
# 	except Exception, e:
# 		print "finally failed at",_level+1,"host"                   
# 		exit(0)
# 	_level+=1
